...
class ExpiredInfoTest {
  ...
  public int[] solution(String today, String[] terms, String[] privacies) {
    ...
    int todayNum =
        monthToDay(yearToMonth(Integer.parseInt(todaySplit[0])))
            + monthToDay(Integer.parseInt(todaySplit[1]))
    ...
    for (int i = 0; i < privacies.length; i++) {
      ...
      int dateNum =
          monthToDay(yearToMonth(Integer.parseInt(date[0])))
              + monthToDay(Integer.parseInt(date[1]))
